...
/*
   Process states from the PDF.
   TERMINATED means the process has been reaped and removed.
*/
typedef enum {
    RUNNING,
    WAITING,
...
/*
   ...
   This keeps all information for one simulated process.
*/
typedef struct {
    int pid;
    int ppid;
    ...
    /* child list required by the PDF */
    int children[MAX_PROCESSES];
    int child_count;

    /* used for wait / wakeup */
    pthread_cond_t cond;
} PCB;

/* one small struct for worker thread information */
typedef struct {
    int thread_number;
    char filename[100];
} WorkerInfo;
...
char last_action[200];

/* snapshot file */
FILE *snapshot_file = NULL;

/* needed so pm_fork / pm_exit / pm_wait / pm_kill can find which worker called them */
...
void add_child_to_parent(int parent_pid, int child_pid)
{
...
void remove_child_from_parent(int parent_pid, int child_pid)
{
    int parent_index = find_process_index(parent_pid);
    int i;
...
void notify_monitor_and_wait(char text[])
{
    ...
    /*
       ...
       This makes sure every change gets its own snapshot.
    */
...
void pm_fork(int parent_pid)
{
    ...
    if (process_count >= MAX_PROCESSES) {
        printf("Process table is full. Cannot create more processes.\n");
    ...
    add_child_to_parent(parent_pid, next_pid);
    ...
    notify_monitor_and_wait(message);
...
void pm_wait(int parent_pid, int child_pid)
{
    ...
    while (1) {
        ...
        /* if no child exists, return immediately */
        if (has_matching_child == 0) {
            pthread_mutex_unlock(&lock);
            return;
        }

        /* if zombie child found, collect it and remove from table */
        if (found_zombie != -1) {
            ...
            remove_child_from_parent(ppid, pid);
        ...
        /* otherwise parent waits until a child exits */
...
/* -------------------------------------------------- */
/* worker thread + script interpreter                 */
/* -------------------------------------------------- */
...
int main(int argc, char *argv[])
{
    ...
    for (i = 0; i < worker_total; i++) {
        ...
        pthread_create(&worker_ids[i], NULL, worker_thread_function, &worker_info[i]);
    }
